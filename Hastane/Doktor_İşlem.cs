...
namespace Hastane
{
    public partial class Doktor_İşlem : Form
    {
        ...
        private void Doktor_İşlem_Load(object sender, EventArgs e)
        {
            
            label8.Text = doktor;
            baglanti.Open();
            SqlCommand ad = new SqlCommand("select * from Doktor where DoktorTc=@a1", baglanti);
            ad.Parameters.AddWithValue("@a1", label8.Text);
        ...
        private void Doktor_İşlem_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
    }
